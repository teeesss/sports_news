...
4. **Substring Collision Prevention**: In-text keyword searches using basic `in` operators can result in false positive classifications due to substring overlap. For example, checking for `"inning"` to classify Baseball matches `"beginning"` in any general news context. Always use regex search with word boundaries (`\b`) and escaping (`re.escape()`).
5. **Indentation and Scope Safety**: Refactoring logic blocks inside multiple nested condition blocks can accidentally disrupt the scoping rules of variables or else conditions (e.g. nested conditions overriding feed promotions). Verify the logical nesting structure matches the architectural intent, particularly after applying chunked regex modifications.
